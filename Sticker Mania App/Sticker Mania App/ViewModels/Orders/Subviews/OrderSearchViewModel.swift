...
class OrderSearchViewModel: ObservableObject {
            ...
            do {
                ...
                for document in allUsersSnapshot.documents {
                    ...
                    if let email = data["email"] as? String,
                       ...
                       (name.lowercased().contains(lowercaseQuery) || email.lowercased().contains(lowercaseQuery)) {
    ...
    func fetchRecentOrders() {
        logger.log("Fetching 5 most recent orders")
        isLoading = true
        errorMessage = nil
        
        orderController.fetchRecentOrders(limit: 5) { [weak self] result in
            DispatchQueue.main.async {
                self?.isLoading = false
                
                switch result {
                case .success(let orders):
                    self?.logger.log("Successfully fetched \(orders.count) recent orders")
                    self?.recentOrders = orders
                case .failure(let error):
                    self?.logger.log("Failed to fetch recent orders: \(error.localizedDescription)", level: .error)
